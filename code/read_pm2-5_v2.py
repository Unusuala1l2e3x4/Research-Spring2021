...
if __name__ == "__main__":
  ...
  map = geopandas.read_file(os.path.join(shapefilesDir, 'USA_states_counties', 'cb_2018_us_state_500k', 'cb_2018_us_state_500k.shp'))
  # map = geopandas.read_file(os.path.join(shapefilesDir, 'USA_states_counties', 'cb_2017_us_county_500k', 'cb_2017_us_county_500k.shp'))
  ...
  for filename in filenames:
    ...
    with plt.style.context(("seaborn", "ggplot")):
      ...
      # levels2 = [0,2.5,5,7.5,10,12.5,15,17.5,20]
      levels2 = np.arange(0,25,2)
      # levels2 = [0,4,8,12,16,20,25]
      contours = plt.contour(lons_1d, lats_1d, bounded_mat, levels=levels2, colors='black', linewidths=0.15, alpha=1) 
      # contours = plt.contour(lons_1d, lats_1d, bounded_mat, levels=len(levels2), colors='black', linewidths=0.2, alpha=1) 
      # contours = plt.contour(lons_1d, lats_1d, bounded_mat, levels=len(levels2), cmap=cmap, linewidths=0.2, alpha=1)
      labels = plt.clabel(contours, fontsize=3*res, colors='black')
      # print(np.array(labels))


      # levels1 = np.arange(np.min(df[unit]),np.max(df[unit]),0.6)
      # plt.contour(lons_1d, lats_1d, bounded_mat, levels=levels1, cmap=cmap, linewidths=1.2, alpha=0.7)
      # levels1 = np.arange(0,25,0.2)
      levels1 = levels2
      plt.contourf(lons_1d, lats_1d, bounded_mat, levels=levels1, cmap=cmap, alpha=0.4)
      ...
      save_plt(plt, outputDir, str(startYear) + '-' + str(endYear) + '_' + regionFile + '_' + str(len(levels1)) + '_' + str(len(levels2)) + '_' + utc_time_filename(), 'png')
