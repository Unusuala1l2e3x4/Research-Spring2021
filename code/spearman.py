...
def main():
  ...
  for columns in columns_list:
    ...
    rho, pval = stats.spearmanr(X_as_mat, X_as_mat)
    # print(rho)
    # print(pval)
    # print()


    df_rho = pd.DataFrame(rho[:len(columns),:len(columns)])
    df_pval = pd.DataFrame(pval[:len(columns),:len(columns)])
    # print(df_rho)
    # print(df_pval)
    # print()

    df_rho.columns = columns
    df_pval.columns = columns

    df_rho.insert(0, 'vars', columns)
    df_pval.insert(0, 'vars', columns)
